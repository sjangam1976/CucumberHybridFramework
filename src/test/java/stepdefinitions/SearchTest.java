package stepdefinitions;

import org.junit.Assert;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;

import factory.DriverFactory;
import io.cucumber.java.en.*;
import pages.SearchPage;

public class SearchTest 
{
	WebDriver driver;
	private SearchPage searchPage;

	@Given("user navigates to search page")
	public void user_navigates_to_search_page() {
		
		driver=DriverFactory.getDriver();
	}

	@When("user enters valid product {string} into search box field")
	public void user_enters_valid_product_into_search_box_field(String validProductText) {
		searchPage =new SearchPage(driver);
		searchPage.enterProductIntoSearchField(validProductText);
	}

	@When("user clicks on search button")
	public void user_clicks_on_search_button() {
		
		searchPage.clickOnSearchButton();
	}

	@Then("user should get valid product displayed in search result")
	public void user_should_get_valid_product_displayed_in_search_result() {
		
		
		Assert.assertEquals("HP LP3065", searchPage.getValidSearchResultText());
	}

	@When("user enters invalid product {string} into search box field")
	public void user_enters_invalid_product_into_search_box_field(String invalidProductText) {
		searchPage =new SearchPage(driver);
		searchPage.enterProductIntoSearchField(invalidProductText);
	}

	@Then("user should get a message about no product matching")
	public void user_should_get_a_message_about_no_product_matching() 
	{
		
		Assert.assertEquals("Products meeting the search criteria", searchPage.getNoProductMessageText());
	}

	@When("user does not enter any product name into search box field")
	public void user_does_not_enter_any_product_name_into_search_box_field() {
		
		searchPage =new SearchPage(driver);
			//intentionally left blank
	}


}
